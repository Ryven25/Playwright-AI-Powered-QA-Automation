...
Use this skill **immediately after** a Playwright test run finishes with failures:

1. Read `test-results/failure-summary.json` if it exists (written by the custom reporter).
2. Read individual failure artifacts under `test-results/` (trace.zip, error-context.md).
3. If the Cursor `postToolUse` hook injected context about a failed `playwright test` command, treat that as an automatic trigger.

Do **not** run this skill when all tests pass.
...
- If the failure is flaky (passes in isolation with `--workers=1`), classify it as test infrastructure — do **not** create a product bug ticket; document the flake and suggest a test fix instead
...
- Link the bug to the originating story (e.g., DS-2) via a Jira issue link