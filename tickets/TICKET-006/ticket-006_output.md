# Test Plan — TICKET-006: TodoMVC Application
...
### TC-201 — Adding a todo with special characters
...
### TC-206 — Editing and pressing Tab / clicking outside to confirm
...
### TC-302 — Todo items have proper ARIA roles or semantic HTML

- **Steps:**
  1. Inspect the DOM structure.
- **Expected result:** Todo list uses `<ul>/<li>` or appropriate `role="list"` / `role="listitem"`. Checkboxes have accessible labels. Edit mode inputs are labelled.
- **Priority:** Medium

### TC-303 — Page is responsive on mobile viewport (375px)

- **Steps:**
  1. Resize browser to 375 × 667 (iPhone SE).
  2. Add, complete, and remove items.
- **Expected result:** No horizontal scrolling. Input field, items, and footer are all usable. Destroy button is reachable via touch/tap.
- **Priority:** Medium

### TC-304 — Performance: adding 50 items does not degrade interaction

- **Steps:**
  1. Programmatically or manually add 50 todo items.
  2. Toggle, edit, and delete items.
- **Expected result:** No perceptible lag (< 100ms per interaction). Scrolling is smooth.
- **Priority:** Low

### TC-305 — XSS: script input renders as text, not executable HTML

- **Test data:** `<img src=x onerror=alert(1)>` and `javascript:void(0)`
- **Steps:**
  1. Add the above strings as todo items.
  2. Inspect the rendered DOM.
- **Expected result:** All strings render as literal text. No DOM injection. No script execution.
- **Priority:** High

### TC-306 — App works correctly with localStorage disabled

- **Preconditions:** Browser localStorage is blocked (via DevTools or settings).
- **Steps:**
  1. Navigate to the app.
  2. Add and complete items.
- **Expected result:** The app either functions normally for the session (without persistence) or gracefully shows an error. It must **not** crash or show a blank page.
- **Priority:** Low
...
1. **"Create a todo list" (AC1)** — The list exists by default on page load. Does "create" mean simply navigating to the page, or is there an explicit "new list" action? (Assumed: navigating to the page IS creating the list.)
2. **"Add items (4)" (AC2)** — Does the number 4 mean "at least 4" or "exactly 4"? Is there a maximum? (Assumed: demonstrate with 4 items; no upper bound.)
3. **"Finish items" (AC3)** — Does this mean complete ONE item to verify the feature, or ALL items? (Test plan covers both: single-item completion and mark-all.)
4. **"Remove item" (AC4)** — Remove by X button only? Or also via "Clear completed"? (Test plan covers both mechanisms.)
5. **Edit functionality** — Not mentioned in ACs but exists in the app (double-click to edit). Covered in edge cases (TC-205, TC-206) since it's a core interaction.
6. **Filters (All / Active / Completed)** — Not mentioned in ACs but integral to the app UX. Covered in edge cases (TC-207, TC-208).
7. **Persistence (localStorage)** — Not mentioned but critical. Covered in TC-209.
...
10. **Accepted browsers** — Assumed Chromium + Firefox + WebKit per Playwright config. Should be confirmed.
